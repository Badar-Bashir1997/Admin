<?php 
 ...
 if(isset($_REQUEST['BtnSubmit']))
    {
      ...
      $Farm=$_REQUEST['Farm'];
        ...
        $q1="SELECT id,remaining,price_per_unit,(SELECT flock_id FROM flock WHERE flock.Flock_name='$Flock1')AS flock_id FROM purchase WHERE remaining>0 AND type='$m_name' AND name='Medicine' ORDER BY id ASC";
 ...
 $confirm_status = mysqli_query($conn,$Query);
       if($confirm_status)
       {
        header("location:medicine_exp.php");
    }
    else
    {
        ?>
                ...
                <label>Medicine Name</label>
                 ...
                 <select class="form-control select2" style="width: 100%;" name="txtName" id="txtName" data-placeholder="Select Medicine Name" onchange="Mdsn_q(this.value);" required >
                  <option></option>
                   ...
                   $query = "SELECT type FROM purchase WHERE remaining>0 AND name='Medicine' GROUP BY type ";
                    $result = mysqli_query($conn,$query);
                     while($row = mysqli_fetch_array($result))
                     {
                     $f_name= $row['type'];
          ...
          <div class="box">
            ...
            <div class="box-body" style="overflow: scroll;">
              <table id="example"class="table table-striped table-bordered" cellspacing="0" width="100%">
                ...
                <tbody>
                  <?php
                    $query = "SELECT *,(SELECT Flock_name FROM flock WHERE flock.flock_id=expences.flock_id)AS flk_id,(SELECT name FROM farm WHERE farm.farm_id=expences.farm_id)AS frm_id FROM expences WHERE e_name='Medicine'";
                    $result = mysqli_query($conn,$query);
